...
exports.convert = async (req, res) => {
  ...
  try {
    ...
    if (resolution && resolution !== 'original') cmd = cmd.size(resolution);
    if (format === 'mp3') cmd = cmd.audioCodec('libmp3lame');
    else if (format === 'aac') cmd = cmd.audioCodec('aac');
    ...
    cmd.toFormat(format)
      .on('start', () => sendProgress(jobId, { type: 'start', message: 'Начало конвертации...' }))
      .on('progress', (progress) => sendProgress(jobId, { type: 'progress', percent: Math.round(progress.percent || 0), timemark: progress.timemark }))
      .on('end', () => { sendProgress(jobId, { type: 'done', percent: 100, downloadUrl: '/api/download/' + outputFilename, filename: outputFilename }); fs.unlink(inputPath, () => {}); })
      .on('error', (err) => { sendProgress(jobId, { type: 'error', message: 'Ошибка конвертации: ' + err.message }); fs.unlink(inputPath, () => {}); })
      .save(outputPath);
  } catch (err) {
